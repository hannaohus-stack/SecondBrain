...
1. Chaeum / KRKK 각 관점에서 전략 제안 1개씩
1. 다음 주 12주 실행을 위한 단 하나의 핵심 행동
   간결하게, 3-4문단.`, qa ); setAiResult(ai); await store.set(`weekly-review-${data.weekDates[0]}`, JSON.stringify({ answers, aiResult: ai }));
...
const ai = await callClaude(
`당신은 Hanna의 Second Brain AI. INFP, Chaeum+KRKK, Brand Builder & Investor 비전. 아침 리뷰: 1.오늘 핵심 의도 한 문장 2.에너지 높이는 리마인더 2개 3.INFP 과잉사고 함정 1개 피하기. 따뜻하지만 직접적으로.`,
qa
);
setAiResult(ai);
...
const ai = await callClaude(
`당신은 Hanna의 Second Brain AI. INFP, Chaeum+KRKK, Brand Builder & Investor 비전. 저녁 리뷰: 1.실행 점수(10점)+이유 2.잘한 것 칭찬 3.내일 핵심 인사이트 1개 4.비전 관점 총평 한 줄. 따뜻하지만 정직하게.`,
`완료: ${done.join(", ")||"없음"}\n미완료: ${undone.join(", ")||"없음"}\n\n${qa}`
);
setAiResult(ai);
...
<style>{`*{box-sizing:border-box} ::-webkit-scrollbar{width:4px} ::-webkit-scrollbar-thumb{background:#ddd;border-radius:2px} textarea,input,select{outline:none} textarea::placeholder,input::placeholder{color:#bbb} @keyframes spin{from{transform:rotate(0)}to{transform:rotate(360deg)}} .nav-btn:hover{background:#f0f0f0 !important} button:disabled{opacity:.4;cursor:not-allowed}`}</style>
